...
`launch_lexdiff.py`를 실행하면 가상환경 생성 → 의존성 설치 → 실행 모드 선택을
한 번에 처리할 수 있습니다. Codespaces, VS Code, 로컬 PC 어디에서든 동일하게
동작하며, 처음 실행 시 필요한 패키지를 자동으로 설치합니다.

```bash
python launch_lexdiff.py
```

스크립트가 실행되면 다음과 같은 메뉴를 선택할 수 있습니다.

- **Tkinter GUI 실행**: 시각화된 데스크톱 앱을 바로 시작합니다.
...
런처를 사용하지 않고 직접 환경을 구성하려면 아래 명령을 순서대로 실행하세요.
